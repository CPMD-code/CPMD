...
!
! Copyright (C) 2016, Bálint Aradi - Adaptation for CPMD by T. Ikeda and M. Boero (2020)
!
...
module dftd3_api
  ...
  type :: dftd3_calc
    ...
    integer :: version
    real(wp), public :: s6, rs6, s18, rs18, alp
    real(wp) :: rthr, cn_thr
...
contains
  ...
  !!
  subroutine dftd3_pbc_dispersion(this, coords, izp, latvecs, n_rep, disp, &
      & grads, stress)
    type(dftd3_calc), intent(in) :: this
    real(wp), intent(in) :: coords(:,:)
    integer, intent(in) :: izp(:)
    ...
    real(wp) :: rtmp3(3)
    integer :: rep_cn(3), rep_vdw(3), n_rep(3)
    ...
    rep_vdw(:) = int(rtmp3) + 1
    ! adapted to cpmd code
    rep_vdw(:) = min(rep_vdw(:), n_rep(:))
    call set_criteria(this%cn_thr, latvecs, rtmp3)
    rep_cn(:) = int(rtmp3) + 1
    ! adapted to cpmd code
    rep_cn(:) = min(rep_cn(:), n_rep(:))
