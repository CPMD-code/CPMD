MODULE pi_prpt_utils
  ...
  USE pimd,                            ONLY: np_local,&
                                             pc_groups,&
                                             supergroup
  USE pi_npt_bomd_utils,               ONLY: pi_npt_bomd
  USE pi_npt_cpmd_utils,               ONLY: pi_npt_cpmd
...
CONTAINS
  ...
  SUBROUTINE pi_prpt
    ...
    CHARACTER(*), PARAMETER                  :: procedureN = 'pi_prpt'
...
! ==--------------------------------------------------------------==
    ...
    IF ( cntl%tqmmm ) CALL stopgm(procedureN,"QMMM NOT IMPLEMENTED",& 
         __LINE__,__FILE__)
    ! ==--------------------------------------------------------------==
    nstate=crge%n
    IF (cntl%tdiag) THEN
    ...
    IF (cntl%tdipd.OR.vdwl%vdwd) THEN
       IF (cntl%tddft) CALL stopgm(procedureN,"cntl%tddft AND DIPOLE DYNAMICS NOT POSSIBLE",& 
            __LINE__,__FILE__)
       lenbk=nxxfun(nstate)
    ...
    ! ==--------------------------------------------------------------==
    IF (cntl%tmdbo) THEN
       CALL pi_npt_bomd(c0,cm,c2,sc0,vpp,gamx,gamy)
    ELSE
       IF (tkpts%tkpnt)CALL stopgm(procedureN,'K-POINTS NOT IMPLEMENTED',& 
            __LINE__,__FILE__)
       CALL pi_npt_cpmd(c0,cm,c2,sc0,gamx,gamy)
    ENDIF
    ! ==--------------------------------------------------------------==
    ...
    RETURN
  END SUBROUTINE pi_prpt
  ! ==================================================================

END MODULE pi_prpt_utils
