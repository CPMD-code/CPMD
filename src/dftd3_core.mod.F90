...
999 continue
  ...
  subroutine pbcncoord(natoms,rcov,iz,xyz,cn,lat,rep_cn,crit_cn)
    ...
    !$omp parallel do private(i,xn,iat,taux,tauy,tauz,tau,dx,dy,dz,r,rco,rr,damp)
    do i=1,natoms
      xn=0.0d0
      do iat=1,natoms
  ...
  subroutine pbcedisp(max_elem,maxc,n,xyz,iz,c6ab,mxc,r2r4,r0ab,&
    ...
    if (version.eq.2)then
      ...
      !$omp parallel do private(iat,jat,c6,ij,taux,tauy,tauz,tau,dx,dy,dz,r2, &
      !$omp r,R0,rr,tmp,damp6,damp8,r6,c8,r8) reduction(+:e6,e8) schedule(static,1)
      do iat=1,n-1
        do jat=iat+1,n
          ! get C6
          call getc6(maxc,max_elem,c6ab,mxc,iz(iat),iz(jat),&
      ...
      !$omp parallel do private(iat,jat,c6,ij,taux,tauy,tauz,tau,dx,dy,dz,r2, &
      !$omp r,R0,rr,tmp,damp6,damp8,r6,c8,r8) reduction(+:e6,e8)
      do iat=1,n
        jat=iat
        ! get C6
        call getc6(maxc,max_elem,c6ab,mxc,iz(iat),iz(jat),&
      ...
      !$omp parallel do private(iat,jat,c6,rxyz,r42,bj_dmp6,bj_dmp8,ij,taux, &
      !$omp tauy,tauz,tau,dxyz,r2,r,rr,r6,c8,r8) reduction(+:e6,e8) &
...
#ifdef __HPC
      ! no-OMP - ionic forces become Infinity on MIC-AVX512 machine
#else
      !$omp parallel do private(iat,c6,dc6iji,dc6ijj,r0,r42,rcovij,taux,tauy, &
      !$omp tauz,tau,rij,r2,r,r6,r7,r8,r9,t6,damp6,t8,damp8,tmp1,tmp2,dc6_rest, &
      !$omp jat,linij) reduction(+:disp,dc6i) schedule(static,1)
#endif
      do iat=1,n
        call get_dC6_dCNij(maxc,max_elem,c6ab,mxc(iz(iat)),&
...
!!!!!!!!!!!!!!!!!!!!!!!
      ...
      ! no-OMP - ionic forces become Infinity on MIC-AVX512 machine
#else
      !$omp parallel do private(iat,c6,dc6iji,dc6ijj,r42,rcovij,R0,taux, &
      !$omp tauy,tauz,tau,rij,r2,r,r4,r6,r7,r8,r9,t6,t8,dc6_rest,jat,linij) &
      !$omp reduction(+:disp,dc6i) schedule(static,1)
#endif
      do iat=1,n
        call get_dC6_dCNij(maxc,max_elem,c6ab,mxc(iz(iat)),&
            & mxc(iz(iat)),cn(iat),cn(iat),iz(iat),iz(iat),&
